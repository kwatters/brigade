...
import com.kmwllc.brigade.stage.AbstractStage2;
 ...
 * 
 */
public class WorkflowWorker2 extends Thread {
  public final static Logger log = LoggerFactory.getLogger(WorkflowWorker2.class.getCanonicalName());
  boolean processing = false;
  boolean running = false;
  boolean error = false;
  private ArrayList<AbstractStage2> stages;

  private final LinkedBlockingQueue<Document> queue;


  WorkflowWorker2(WorkflowConfig2<StageConfig2> workflowConfig, LinkedBlockingQueue<Document> queue) throws ClassNotFoundException {
    // set the thread name
    this.setName("WorkflowWorker-" + workflowConfig.getName());
    this.queue = queue;
    stages = new ArrayList<>();
    for (StageConfig2 stageConf : workflowConfig.getStages()) {
      ...
      try {
        AbstractStage2 stageInst = (AbstractStage2) sc.newInstance();
  ...
  public void processDocumentInternal(Document doc, int stageOffset) throws Exception {
    // TODO: what to do...
    int i = 0;
    for (AbstractStage2 s : stages.subList(i, stages.size())) {
  ...
  public void addStage(AbstractStage2 stage) {
    stages.add(stage);
  }

  public void flush() {
    for (AbstractStage2 s : stages) {
      s.flush();
    }
  }
